...
use nativepkg_cli::format::Format;
use nativepkg_core::npm::{InitSystem, InstallStrategy};
use nativepkg_core::plan::BuildPlan;
use nativepkg_core::resolve::{Overrides, ResolvedConfig};
...
fn scripts_in(format: Format, path: &Path) -> String {
    ...
    match format {
        Format::Deb => {
            let package = nativepkg_deb::read::parse(&bytes).expect("a readable .deb");
            package
                .script_bodies
                .values()
                .cloned()
...
fn the_executable_symlink_points_at_something_in_the_package() {
    ...
    for format in Format::ALL {
        let variables = format.variables(&config, "0.1.0").expect("vocabulary");
        let generated: Vec<_> = nativepkg_core::build::service_files(&config)
            .expect("placement")
            .into_iter()
            .map(|service| {
                let source = nativepkg_core::template::builtin(service.template)
                    .expect("a built-in template");
                let text = nativepkg_core::template::render(service.template, source, &variables)
                    .expect("renders");
                (service, text)
            })
            .collect();
        let derived = nativepkg_core::build::with_generated(&plan, generated).expect("plan");
        ...
        let nativepkg_core::plan::EntryKind::Symlink { target } = &link.kind else {
...
fn a_systemd_package_declares_its_log_directory_to_tmpfiles() {
    ...
    let placed = nativepkg_core::build::service_files(&config).expect("placement");
    let fragment = placed
        .iter()
    ...
    let variables = Format::Deb.variables(&config, "0.1.0").expect("vocabulary");
    let source = nativepkg_core::template::builtin("tmpfiles.conf").expect("built in");
    let text =
        nativepkg_core::template::render("tmpfiles.conf", source, &variables).expect("renders");
...
fn auto_does_not_include_upstart_but_an_explicit_request_does() {
    ...
    let auto: Vec<&str> = nativepkg_core::build::service_files(&config)
        .expect("placement")
        .iter()
        .map(|s| s.template)
        .collect::<Vec<_>>()
    ...
    let explicit: Vec<&str> = nativepkg_core::build::service_files(&config)
        .expect("placement")
        .iter()
        .map(|s| s.template)
        .collect();
