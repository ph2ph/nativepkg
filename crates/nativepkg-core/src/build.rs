...
use crate::plan::{BuildPlan, Description, Destination, Identity, PlanMetadata, PlannedFile};
use crate::resolve::{ResolvedConfig, Warning};
use crate::timestamp::{self, Timestamp, TimestampSource};
...
pub fn plan(
    ...
    let sources: Vec<&Path> = files
        ...
        .filter_map(|f| match &f.content {
            crate::plan::FileContent::FromPath { path, .. } => Some(path.as_path()),
            _ => None,
        })
        .collect();
...
fn refuse_missing_entrypoints(config: &ResolvedConfig, files: &[PlannedFile]) -> Result<()> {
    for (destination, executor) in crate::collect::executed_destinations(config) {
