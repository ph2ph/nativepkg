...
impl Variables {
    ...
    pub fn for_config(
        ...
        let init = match config.init {
            crate::npm::InitSystem::Auto => "auto",
            crate::npm::InitSystem::Upstart => "upstart",
            crate::npm::InitSystem::Systemd => "systemd",
            crate::npm::InitSystem::Sysv => "sysv",
            crate::npm::InitSystem::None => "none",
        };
        let strategy = match config.install_strategy {
            crate::npm::InstallStrategy::Auto => "auto",
            crate::npm::InstallStrategy::Copy => "copy",
            crate::npm::InstallStrategy::NpmInstall => "npm-install",
        };
        ...
        Self::new()
            ...
            .with(
                "package_description_shell",
                crate::text::shell_double_quoted(synopsis),
            )
            .with("package_maintainer", &config.maintainer)
            .with(
                "package_maintainer_shell",
                crate::text::shell_double_quoted(&config.maintainer),
            )
            .with(
                "package_dependencies",
...
const BUILTIN: &[(&str, &str)] = &[
    ...
    ("tmpfiles.conf", include_str!("../templates/tmpfiles.conf")),
];
...
mod tests {
    ...
    fn the_published_vocabulary_is_what_a_configuration_populates() {
        ...
        let (config, _) =
            crate::resolve(&manifest, &crate::resolve::Overrides::default()).expect("resolves");
