...
use crate::core::name::{EntryPoint, ExecutableName, InstallDir, PackageName, UnixName};
use crate::core::npm::{
    Author, Entrypoints, InitSystem, InstallStrategy, Manifest, Settings, Templates, command_binary,
};
use crate::core::text;
use crate::core::version::{MappedVersion, VersionSpec};
...
impl ResolvedConfig {
    ...
    pub fn architecture_parsed(&self) -> Result<crate::core::arch::Architecture> {
        crate::core::arch::Architecture::parse(&self.architecture)
    }
}
    ...
    Ok((
        ResolvedConfig {
            ...
            workspace_roots: manifest
                .workspaces
                .as_ref()
                .map(crate::core::npm::Workspaces::directory_prefixes)
                .unwrap_or_default(),
            homepage,
            license,
            dependencies,
