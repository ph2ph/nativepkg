...
use crate::core::scratch::{OutputFile, ScratchFile};
...
pub struct Options {
    ...
    /// ([`crate::core::build::systemd_preset_entry`]): Arch's `99-default.preset` says
...
fn with_preset(plan: &BuildPlan, options: &Options) -> Result<BuildPlan> {
    ...
    files.push(crate::core::build::systemd_preset_entry(service).map_err(Error::Core)?);
    BuildPlan::new(
        plan.identity.clone(),
        files,
        plan.timestamp,
