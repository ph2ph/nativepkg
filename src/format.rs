...
use crate::core::template::Variables;
...
impl Format {
    ...
    pub fn unsupported(self, init: InitSystem) -> Option<String> {
        match self {
            Self::Arch => crate::arch::install::unsupported_init(init).map(|name| {
                format!(
    ...
    pub fn build(
        ...
        let path = match self {
            Self::Deb => {
                ...
                for script in crate::deb::scripts::Script::all() {
                    let text = crate::deb::scripts::compose(script, init, strategy, variables)
                ...
                let required = crate::deb::scripts::required_packages(init, strategy);
                ...
                let options = crate::deb::Options {
                    maintainer_scripts: scripts,
                    triggers,
                    ..crate::deb::Options::default()
                };
                crate::deb::build(&plan, &options, output_dir).context("writing the .deb")?
            }
            Self::Rpm => {
                let mut scripts = Vec::new();
                for slot in crate::rpm::scriptlets::Scriptlet::all() {
                    let text = crate::rpm::scriptlets::compose(slot, init, strategy, variables)
                ...
                let required = crate::rpm::scriptlets::required_packages(init, strategy);
                let plan = with_dependencies(plan, &required);
                ...
                // comes up.
                let options = crate::rpm::Options {
                    maintainer_scripts: scripts,
                    preset_service: crate::rpm::scriptlets::uses_preset(init)
                        .then(|| plan.identity.package_name.clone()),
                };
                crate::rpm::build(&plan, &options, output_dir).context("writing the .rpm")?
            }
            Self::Arch => {
                let scriptlet = crate::arch::install::compose(init, strategy, variables)
                ...
                let options = crate::arch::Options {
                    install_scriptlet,
                    preset_service: crate::arch::install::uses_preset(init)
                        .then(|| plan.identity.package_name.clone()),
                };
                crate::arch::build(plan, &options, output_dir)
