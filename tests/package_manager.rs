...
impl Project {
    ...
    fn build(&self, extra: &[&str]) -> nativepkg::deb::read::Package {
        ...
        let path = std::fs::read_dir(&out)
            ...
            .expect("a .deb");
        nativepkg::deb::read::parse(&std::fs::read(path).expect("read")).expect("a readable .deb")
    }
}
...
fn postinst(pkg: &nativepkg::deb::read::Package) -> String {
    pkg.script_bodies
        .get("postinst")
        .cloned()
