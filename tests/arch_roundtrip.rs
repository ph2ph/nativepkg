...
use nativepkg::arch::{Options, build_bytes, mtree, pkginfo};
...
fn the_manifest_lists_the_install_scriptlet_when_there_is_one() {
    let options = nativepkg::arch::Options {
        install_scriptlet: Some(b"post_install() {\n  :\n}\n".to_vec()),
        ..Options::default()
    };
    let bytes = nativepkg::arch::build_bytes(&sample_plan(), &options).expect("build succeeds");
...
fn a_preset_service_ships_its_enabling_policy() {
    ...
    let plain = nativepkg::arch::build_bytes(&sample_plan(), &Options::default()).expect("build");
    assert!(
