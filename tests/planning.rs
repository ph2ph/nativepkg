...
use nativepkg::core::build::plan_at;
use nativepkg::core::npm::InstallStrategy;
use nativepkg::core::plan::{EntryKind, FileContent};
use nativepkg::core::resolve::{Overrides, ResolvedConfig, Warning};
use nativepkg::core::timestamp::Timestamp;
use nativepkg::core::{Manifest, resolve};
...
fn every_entry_carries_the_plan_timestamp_and_root_ownership() {
    ...
    assert_eq!(nativepkg::core::plan::BuildPlan::UID, 0);
    assert_eq!(nativepkg::core::plan::BuildPlan::GID, 0);
}
...
fn plans_round_trip_through_json() {
    ...
    let back: nativepkg::core::plan::BuildPlan =
...
fn rebuild_cost_at_fifty_thousand_entries() {
    use nativepkg::core::plan::{Destination, PlannedFile};
    ...
    fx.write("app.js", "console.log(1);\n");
    let (small, _, _) =
        nativepkg::core::build::plan(&fx.config(), &fx.root, &inputs(&["app.js"])).expect("plan");
    files.extend(small.files.iter().cloned());
    let big = nativepkg::core::plan::BuildPlan::new(
    ...
    let rebuilt = nativepkg::core::plan::BuildPlan::new(
        big.identity.clone(),
        big.files.clone(),
        big.timestamp,
        big.metadata.clone(),
