//! Fitness functions for the shape of the crate: every place the core module names a package
...
/// The core module's source files minus their test modules: a test may name a format freely.
fn scannable() -> Vec<(String, Vec<(usize, String)>)> {
    let root = Path::new(concat!(env!("CARGO_MANIFEST_DIR")));
    let mut out = Vec::new();

    let mut stack = vec![root.join("src").join("core")];
...
/// Every process the crate may spawn, and why. An allowlist: the blacklist it replaced was
...
/// not hide one.
#[test]
fn every_process_the_crate_spawns_is_registered() {
    let src = Path::new(concat!(env!("CARGO_MANIFEST_DIR"))).join("src");
...
/// The dependency arrow points from the backends into the core, never back and never sideways:
/// a single edge the other way would let one format's quirks reach the plan every other format
/// reads. In one crate that edge is a `crate::<module>` path, so that is what is checked — in
/// code only, since a comment may mention a sibling freely.
#[test]
fn the_core_and_each_backend_reach_no_sibling_module() {
    let root = Path::new(concat!(env!("CARGO_MANIFEST_DIR"))).join("src");
    let forbidden: &[(&str, &[&str])] = &[
        ("core", &["crate::deb", "crate::rpm", "crate::arch"]),
        ("deb", &["crate::rpm", "crate::arch"]),
        ("rpm", &["crate::deb", "crate::arch"]),
        ("arch", &["crate::deb", "crate::rpm"]),
    ];
    let mut violations = Vec::new();
    for (module, siblings) in forbidden {
        let mut stack = vec![root.join(module)];
        while let Some(dir) = stack.pop() {
            for entry in std::fs::read_dir(&dir).expect("module directory readable") {
                let path = entry.expect("directory entry").path();
                if path.is_dir() {
                    stack.push(path);
                    continue;
    ...
    assert!(
        violations.is_empty(),
        "these lines reach a sibling module:\n{}",
        violations.join("\n")
    );
}
...
fn every_registered_needle_still_exists() {
    ...
    for (file, needle, _) in REGISTER {
        let path = if file.contains('/') {
            ...
            root.join("src").join("core").join(file)
        };
